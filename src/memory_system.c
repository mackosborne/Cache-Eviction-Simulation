...
// memory_system.h.
//
// ...Mack's Change - including math library for logs...
#include <math.h>
...
struct cache_system *cache_system_new(uint32_t line_size, uint32_t sets, uint32_t associativity)
{
    ...
    // TODO: calculate the index bits, offset bits and tag bits. ------------------------------Done by Mack
    cs->index_bits = log2(sets);      //log(number of lines/associativity) --associativity is #lines per set * sets = # lines, assoc. cancels left with sets
    cs->offset_bits = log2((sets*associativity)*line_size);   //log(cacheSize)
    cs->tag_bits = (line_size) -(log2(sets)) + (log2((sets*associativity)*line_size)); //line size-index bits +offset bits
                                                ...
                                                uint32_t tag)
{
    ...
    //for lines in set =  for associativity
    //for(int i = 0; i<cache_system->associativity;i++){
      //  if(tag == set_idx[i].cache_line.tag){
        //    int *pointer = NULL;
          //  pointer = *cache_lines[i];
            //return pointer;
            //break;
    //}
